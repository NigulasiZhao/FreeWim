...
public class SpeedTestClient : ISpeedTestClient
{
    ...
    public Settings GetSettings()
    {
        ...
        if (serversConfig.Servers.Count <= 0) throw new InvalidOperationException("SpeedTest does not return any server");
    ...
    public int TestServerLatency(Server server, int retryCount = 3)
    {
        ...
        for (var i = 0; i < retryCount; i++)
        {
            ...
            if (!testString.StartsWith("test=test")) throw new InvalidOperationException("Server returned incorrect test string for latency.txt");
        }
    ...
    private static IEnumerable<string> GenerateUploadData(int retryCount)
    {
        ...
        for (var sizeCounter = 1; sizeCounter < MaxUploadSize + 1; sizeCounter++)
        {
            ...
            for (var i = 0; i < size; ++i) builder.Append(Chars[random.Next(Chars.Length)]);

            for (var i = 0; i < retryCount; i++) result.Add(builder.ToString());
        }
