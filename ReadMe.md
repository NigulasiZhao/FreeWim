...
| **设备流量占比分析** | 实时查询 | 支持查询所有设备或单个设备的小时级流量占比，提供可视化的时段流量分布图表。 |
...
| **流量监控看板** | `/trafficstatistics` | **设备流量统计**，查看各设备的网络使用情况、每日流量趋势、时段流量占比分析。 |
...
**Q: 如何配置华硕路由器流量监控？**
A: 在 `appsettings.json` 中配置 AsusRouter 节点，填写路由器 IP、管理账号和密码即可。确保路由器启用了 Web 访问功能。系统支持查看设备列表、每日流量趋势、时段流量占比等多维度数据分析。
...
**Q: 数据库初始化包含哪些表？**
A: 系统启动时会通过 `DatabaseInitializerService` 自动创建所需的数据库表，包括：考勤记录、禅道任务、加班记录、测速记录、事件信息、Git提交记录、华硕路由器设备、设备流量统计（按小时）、设备流量详情统计（按应用）等共11张核心表。初始化过程在应用启动时自动执行。
